...
2. Clean and normalize data (standardize IDs, replace missing values,
   drop irrelevant columns)
3. Engineer features for modeling (rolling averages, fixture difficulty
   adjustments, minutes likelihood)
...
class FPLFeatureEngineer:
    ...
    def clean_and_normalize_data(self):
        ...
        for col in numeric_cols:
            if col in self.results_df.columns:
                self.results_df[col] = pd.to_numeric(
                    self.results_df[col], errors="coerce"
                ).fillna(0)

        # Ensure non-negative points (data validation showed negative points)
        if "total_points" in self.results_df.columns:
            self.results_df["total_points"] = self.results_df["total_points"].clip(
                lower=0
            )
        ...
        if "event" in self.fixtures_df.columns:
            self.fixtures_df["gameweek"] = pd.to_numeric(
                self.fixtures_df["event"], errors="coerce"
            )

        for col in ["team_h_difficulty", "team_a_difficulty"]:
            if col in self.fixtures_df.columns:
                self.fixtures_df[col] = pd.to_numeric(
                    self.fixtures_df[col], errors="coerce"
                ).fillna(3)
    ...
    def create_fixture_difficulty_features(self):
        ...
        home_fixtures = self.fixtures_df[
            ["gameweek", "team_h", "team_h_difficulty"]
        ].copy()
        ...
        away_fixtures = self.fixtures_df[
            ["gameweek", "team_a", "team_a_difficulty"]
        ].copy()
        ...
        results_with_teams = self.results_df.merge(
            player_teams, on="player_id", how="left"
        )
    ...
    def create_minutes_likelihood_features(self):
        ...
        for window in self.rolling_windows:
            ...
            if not minutes_when_played.empty:
                ...
                self.results_df[col_name] = (
                    self.results_df["player_id"]
                    .map(avg_minutes.groupby("player_id").tail(1))
    ...
    def create_position_and_price_features(self):
        ...
        player_info = self.players_df[
            ["player_id", "position", "now_cost", "team_id"]
        ].copy()
        ...
        self.results_df = self.results_df.merge(
            player_info, on="player_id", how="left", suffixes=("", "_player")
        )

        # Use the team_id from player info if not already present
        if "team_id_player" in self.results_df.columns:
            self.results_df["team_id"] = self.results_df["team_id"].fillna(
                self.results_df["team_id_player"]
            )
    ...
    def finalize_features_dataset(self):
        ...
        existing_columns = [
            col for col in feature_columns if col in self.results_df.columns
        ]
        ...
        print(
            f"  ✅ Final dataset: {len(self.features_df)} rows, "
    ...
    def validate_features(self):
        ...
        missing_cols = [
            col for col in required_cols if col not in self.features_df.columns
        ]
