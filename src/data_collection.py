...
class FPLDataCollector:
    ...
    def collect_bootstrap_data(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for player in data["elements"]:
            ...
            player_record = {
                ...
                "creativity": (
                    float(player["creativity"]) if player["creativity"] else 0
                ),
                ...
                "points_per_game": (
                    float(player["points_per_game"]) if player["points_per_game"] else 0
                ),
                "ep_next": float(player["ep_next"]) if player["ep_next"] else 0,
                "ep_this": float(player["ep_this"]) if player["ep_this"] else 0,
                "value_form": (
                    float(player["value_form"]) if player["value_form"] else 0
                ),
                "value_season": (
                    float(player["value_season"]) if player["value_season"] else 0
                ),
        ...
        players_df = players_df.merge(
            teams_df[["team_id", "team_name", "short_name"]], on="team_id", how="left"
        )
    ...
    def collect_gameweek_data(self, max_gameweeks: int = 38) -> pd.DataFrame:
        ...
        if len(results_df) > 0:
            ...
            results_df = results_df.sort_values(["gameweek", "player_id"]).reset_index(
                drop=True
            )
